...
func _ready() -> void:
    # 🔄 FIX: Enforce continuous layout container sizes programmatically
    if has_node("AffinityPanelRoot"): get_node("AffinityPanelRoot").custom_minimum_size = Vector2(150, 40)
    if has_node("ElementalRoot"): get_node("ElementalRoot").custom_minimum_size = Vector2(100, 40)
...
func bind_battler(new_battler: Battler) -> void:
    ...
    # Force immediate calculation instead of letting it render on unassigned frame
    _refresh_panel()
...
func _update_elemental_and_status_icons() -> void:
    ...
    # Only show the sub-bar items if there are active modifiers to
